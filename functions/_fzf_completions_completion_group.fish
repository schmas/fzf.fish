function _fzf_completions_completion_group -d "Determine completion group"
    set -l path_candidate (_fzf_completions_path_to_complete)
    # Null means that either $path is empty or is not a directory
    set -l is_null (ls -A -- $path_candidate 2> /dev/null | string collect)
    set -l complist (string escape -- (_fzf_completions_expand_tilde (_fzf_completions_parse_complist)))
    # Directories
    ...
    if test -n "$is_null"; and eval "$dir_test"
        ...
        echo files
        # Options
    else if string match --regex --quiet -- '\h+\-+\h*$' $fzf_completions_commandline
